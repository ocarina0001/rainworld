...
namespace RainWorld
{
    public class IncidentWorker_KarmaFlower : IncidentWorker
    {
		...
		protected override bool CanFireNowSub(IncidentParms parms)
		{
			...
			if (!PlantUtility.GrowthSeasonNow(map, VariousDefOf.OCARINA_Plant_KarmaFlower)) // help here
			{
				return false;
			}
		...
		private bool CanSpawnAt(IntVec3 c, Map map)
		{
			...
			if (!c.Standable(map) || c.Fogged(map) || map.fertilityGrid.FertilityAt(c) < VariousDefOf.OCARINA_Plant_KarmaFlower.plant.fertilityMin || !c.GetRoom(map).PsychologicallyOutdoors || c.GetEdifice(map) != null || !PlantUtility.GrowthSeasonNow(c, map, VariousDefOf.OCARINA_Plant_KarmaFlower))
			{
				return false;
			}
